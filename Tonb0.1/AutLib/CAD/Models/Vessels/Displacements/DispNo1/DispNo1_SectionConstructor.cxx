...
namespace AutLib
{
	...
	static Stl_Vector<Pnt2d> UniformDiscretLine
	(
		const Pnt2d& P0,
		const Pnt2d& P1,
	...
	{
		Stl_Vector<Pnt2d> Points(NbSegments + 1);
	...
	(
		...
		Stl_Vector<Pnt2d>& Points
	)
	{
		Standard_Integer
			K = 0;

		Stl_Vector<Pnt2d> Inner;
	...
	std::shared_ptr<DispNo1_SectionCtrlPts> DispNo1_HullPatch::CreateRawSection
	(
	...
	)
	{
		...
		auto MaxSideSlope = atan(DispNo1::MAX_SIDESLOPE_COEFF*Width / Height);
		...
		auto MaxDeadRiseHeight = DispNo1::MAX_DEADRISE_COEFF*Height;
		...
		Stl_Vector<Pnt2d> Points(NbNetRows().Value());
		...
		auto newSection = std::make_shared<DispNo1_SectionCtrlPts>();
		Assemble
		(
			xSection,
			Points,
	...
	std::shared_ptr<DispNo1_SectionCtrlPts> DispNo1_HullPatch::CreateRawStem() const
	{
		auto Section = NbNetColumns().Value() - 1;
		...
		auto MaxPoints = NbNetRows().Value() - 1;

		Stl_Vector<Pnt2d> Points = UniformDiscretLine
		(
			P0,
			P1,
			MaxPoints - 1
	...
	std::shared_ptr<DispNo1_SectionCtrlPts> DispNo1_HullPatch::CreateRawStemPaired()
	{
		auto Section = NbNetColumns().Value() - 2;
		...
		auto MaxSideSlope = atan(DispNo1::MAX_SIDESLOPE_COEFF*ySection / Height);
		auto SideSlopeAngle = SideSlope * MaxSideSlope;
		...
		y0 = MAX(y0, ySection*DispNo1::MIN_DIST_COEFF);
		...
		Stl_Vector<Pnt2d> Points = UniformDiscretLine(P0, P1, MaxPoints - 1);
	...
	void DispNo1_HullPatch::CreateRawSections()
	{
