...
void AutLib::DispNo1_HullPatch::SetupControlNet()
{
	Standard_Integer
		NbSections,
		NbRows,
	...
	NbSections = NbNetColumns().Value();
	NbRows = NbNetRows().Value();
	...
	)
	{
		...
		const auto& ControlPoints = SectionPtr->CtrlPts;
...
namespace AutLib
{
	...
	(
		Standard_Integer nI,
		Standard_Integer nJ,
		Stl_Vector<Standard_Real>& uKnots,
		Stl_Vector<Standard_Real>& vKnots
	)
	{
		if (uKnots.size() != nI)
		{
	...
	(
		...
		const Stl_Vector<Standard_Real>& Parameters,
		Stl_Vector<Standard_Real>& Knots
	)
	{
...
void AutLib::DispNo1_HullPatch::SetupPatch()
{
	Stl_Vector<Standard_Real>
		uParameters,
		vParameters;
	...
	auto NbSections = NbNetColumns().Value();
	auto NbRows = NbNetRows().Value();
