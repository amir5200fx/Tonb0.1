...
namespace AutLib
{
	...
	namespace DispNo1
	{
		...
		protected:
			...
			{
				if (NOT INSIDE(n, dimension::MIN_NB_SECTIONS, dimension::MAX_NB_SECTIONS))
				{
					FatalErrorIn("void AllocateMemory(const Standard_Integer n)")
						<< " Invalid Nb of Sections; nb. of Sections = " << n << endl
						<< " Min nb. of sections = " << dimension::MIN_NB_SECTIONS << endl
						<< " Max nb. of sections = " << dimension::MAX_NB_SECTIONS << endl
						<< abort(FatalError);
				}
		...
		typedef std::shared_ptr<Ship_SectionCtrlPts> sectionCtrlPtsPtr;
