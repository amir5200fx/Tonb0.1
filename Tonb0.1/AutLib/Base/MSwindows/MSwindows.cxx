...
const ::AutLib::word AutLib::MSwindows::typeName(AutLib::MSwindows::typeName_());
::AutLib::debug::debugSwitch AutLib::MSwindows::debug(std::string(AutLib::MSwindows::typeName_()), 0, "");
...
namespace AutLib
{
	...
	namespace MSwindows
	{
		...
		//- AutLib's file io (see src/OpenFOAM/db/IOstreams/Sstreams/OSwrite.C)
	...
	inline void MSwindows::removeQuotes(std::string & arg)
	{
		std::size_t pos;
	...
	inline void MSwindows::toUnixSlash(string & arg)
	{
		arg.replaceAll("\\", "/");
	...
	template<class T>
	inline MSwindows::AutoArray<T>::AutoArray(const unsigned long arrayLength)
	...
	inline bool MSwindows::DirectoryIterator::isValid() const
	{
	...
	inline bool MSwindows::DirectoryIterator::hasNext() const
	{
		assert(isValid());
	...
	inline const fileName & MSwindows::DirectoryIterator::next()
	{
		assert(hasNext());
	...
	{
		const bool success = ::SetEnvironmentVariable(envName.c_str(), value.c_str());
		return success;
	}
	...
	{
		// Not supported but assume worst case for AutLib::dynamicCode::checkSecurity
		return true;
	}
	...
	//	if (isDir(searchDir))
	//	{
	//	fileName fullName = searchDir/AUTversion/name;
	//	if (isFile(fullName))
	//	{
	...
	//	if (isDir(searchDir))
	//	{
	//	fileName fullName = searchDir/"site"/AUTversion/name;
	//	if (isFile(fullName))
	//	{
	//	results.append(fullName);
	//	if (findFirst)
	...
	//	{
	//	std::cerr
	//	<< "--> FOAM FATAL ERROR in AutLib::findEtcFiles() :"
	...
	fileName findEtcFile(const fileName& name, bool mandatory)
	{
		...
		if (mandatory)
		{
			cerr << "--> FOAM FATAL ERROR in AutLib::findEtcFile() :"
				" could not find mandatory file\n    '"
				<< name.c_str() << "'\n\n" << std::endl;
			::exit(1);
		}
	...
	bool cp(const fileName& src, const fileName& dest)
	{
		...
		else if (src.type() == fileName::DIRECTORY)
		{
			...
			for (label i = 0; i < (contents).size(); i++)
			{
				if (MSwindows::debug)
				{
			...
			for (label i = 0; i < (subdirs).size(); i++)
			{
				if (MSwindows::debug)
				{
